...
## Files
- `index.html`, `app.js` — UI: polaroid wall, add-yourself flow (camera + 20 s voice note + signature), per-student page with sticky-note impressions, the shared drawing wall (*Peretele cu desene*), and the shared photo gallery (*Amintiri*) with its full-screen viewer.
...
with the polaroid wall, the drawing wall and the photo grid, a page per classmate under
`elevi/` with their portrait, voice note, signature and every impression, `amintiri.html`
with the photos full-size, and `media/` holding the actual files — the wall flattened as
`media/perete.png`, and each child's layer on its own under `media/perete/`. There is **no JavaScript in
...
impressions as the same coloured sticky notes, the drawing wall on a page of its own,
then the photos. Bookmarked by
...
| GET | `/drawings` | — | `[{ id, from, url, createdAt }]`, **oldest first** — the order the layers are painted in |
| POST | `/drawings` | `multipart/form-data`: `from`, `drawing` (transparent PNG, 1200×1500) | `{ id }` |
...
## Drawing wall
One wall for the whole class, under the polaroids. **Desenează pe perete** opens a
full-screen easel with the wall as it stands, a row of colour swatches plus a
rainbow one that opens the browser's own colour picker (`<input type="color">`),
three brush widths, an eraser, undo and clear. The child signs or draws with a
finger (or a mouse), types a name, and pastes it on.

Every visit is saved as its own **layer**: a transparent PNG the size of the wall
(1200×1500, portrait so a phone gets a drawing area and not a letterbox), plus the
name. The page draws the layers oldest-first onto one canvas; that is the wall.
Layers rather than one ever-growing bitmap for three reasons: nobody can paint over
a classmate — the eraser only takes ink off the layer being drawn, and the wall shows
through; two phones drawing at once both land, with no last-write-wins; and the
export can keep every child's contribution as a separate file. Strokes are recorded
in wall pixels, not screen pixels, and replayed for undo, so a drawing made on a
small screen is as crisp as any other. Closing the easel with unsaved strokes asks
first.

Locally the layers go to the `drawings` object store (DB version 3; an older album
upgrades in place). The installed app gets a **Desenează** shortcut
(`?actiune=deseneaza`) next to the two it already had.
...
- Rate-limit `POST` and cap upload size (~1 MB portrait, ~1 MB audio, ~200 KB signature, ~2 MB gallery photo, ~500 KB wall layer) — the client resizes before upload, but do not trust it.
